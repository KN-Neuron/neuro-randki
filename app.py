...
def _sim_to_score(sim: float) -> int:
    """Map cosine similarity [-1, 1] → percentage 0-100 via soft-sigmoid."""
...
def _save_embedding(user_id: int, signal: np.ndarray):
    ...
    db = get_db()
    db.execute('UPDATE user SET embedding = ? WHERE id = ?', (emb_json, user_id))
    db.commit()
    return emb
...
def _load_embedding(user_id: int) -> np.ndarray | None:
    db = get_db()
    row = db.execute('SELECT embedding FROM user WHERE id = ?', (user_id,)).fetchone()
...
def solo():
    if request.method == 'POST':
        ...
        flask_session.update(mode='solo', solo_user_id=uid, solo_nick=nick)
        return redirect(url_for('waiting'))
...
def waiting():
    ...
    return render_template('waiting.html',
                           player=player, nick=nick, nick1=nick1, mode='paired',
                           next_url=url_for('task'))
...
def task():
    ...
    if mode == 'solo':
        ...
        player = flask_session.get('current_player', 1)
        user_id = flask_session.get('user1_id') if player == 1 else flask_session.get('user2_id')
        next_url = (url_for('waiting') if player == 1 else url_for('results'))

    return render_template('task.html', user_id=user_id, next_url=next_url)
...
def results():
    ...
    res = db.execute('SELECT score, similarity FROM result WHERE session_id = ?', (sid,)).fetchone()
    if res:
        score, similarity = res['score'], res['similarity']
    else:
        u1 = flask_session.get('user1_id')
        u2 = flask_session.get('user2_id')
        emb1 = _load_embedding(u1)
        emb2 = _load_embedding(u2)
        ...
        db.execute(
            'INSERT INTO result (session_id, score, similarity) VALUES (?, ?, ?)',
...
def solo_results():
    nick = flask_session.get('solo_nick', 'Gracz')
    return render_template('solo_results.html', nick=nick,
                           graph_url=url_for('graph'))
...
def api_eeg_start():
    ...
    hw = eeg_mod.start_recording(int(user_id))
    return jsonify(status='recording', hardware=hw)
...
def api_eeg_stop():
    ...
    # Advance player counter for paired mode so /results knows both are done
    mode = flask_session.get('mode', 'paired')
    if mode == 'paired':
        player = flask_session.get('current_player', 1)
        if player == 1:
...
def api_graph_data():
    ...
    embeddings = []
    for r in rows:
        nodes.append({'id': r['id'], 'nickname': r['nickname'], 'is_solo': bool(r['is_solo'])})
